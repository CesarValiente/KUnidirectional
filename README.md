...
This app comes up as a support material to my talk **Unidirectional data flow architecture on Android using Kotlin**.

Talking about architecture is always tough, so as support for the talk and slides that I created, I decided to create an app where I show what I explain in the talk/slides, so is easier to understand everything; and *getting your hands dirty* is always fun!
...
Why the *K* ? because we use [Kotlin](http://kotlinlang.org/) as programming language, apart of being such a nice and enjoyable language, we take advantage of the really great features this language provide us for building our app.
...
Items are shown in a list where they can be reorder, deleted and set as favorites.
...
3. *Do you want to have a look to my code? yes sure!* but here the problem is not that maybe you don't know the programming language, or the framework used, or libraries, is that the architecture of the app, the components, the modules, are completely different that the ones you are used to.
...
We know that, we know that pain, how it feels, and we wanted to avoid that if possible; we wanted:
...
4. To help and being helped!
5. To work/help on another platform without the feeling that you really don't know where to start working nor what to do.

Having the same architecture in all our platforms, make our talks between clients amazing, we all are in the same boat, we know how to drive that boat and how to improve it, all of us.
...
Nope, this code is inspired on what we do and how we do, but is much more simpler, so that's better for educational purposes like we have here :wink:
...
<br/>If you are coming from Clean Architecture, **Store is our Domain**. Store has several and important components:
	...
	2. [**State**](http://redux.js.org/docs/introduction/CoreConcepts.html#core-concepts) this entity has everything what we need to use, is a simple and immutable data structure. 
	...
	<br /> Dispatcher can be implemented on many ways, as a simple function, as a class with a list of subscribers, using external libraries that follow an event/subscriber pattern or others that follow observer pattern, etc. 
	...
	4. [**Reducers**](http://redux.js.org/docs/basics/Reducers.html) are pure functions that given an action and the current state of the app, apply this action to that state, and generate a completely new state (remember, state is an immutable object). The logic of what we have to do resides here, for instance, when we want to create an item we do:
 ...
 * [**Persistence**](http://redux.js.org/docs/advanced/Middleware.html) is the only middleware we have in this sample app, middleware is subscribed to handle actions, once that receives an action, is going to do something with it, in our case, since is the persistence layer, we are going to do something in the DB, a CRUD operation. 
 <br /> Once that we have done what we have to do, we can (or not, is optional) send a new action with data that will be dispatched by the dispatcher and handled later for those who are interested in that action. 
 <br/> For instance when we fetch data from the DB, is the perfect scenario where we send a new action with he new items that we have fetched.<br/>
 <br/>A pretty cool thing of middleware is that we can *"enable or disable"* it, as you can see in [this video that I've uploaded](https://goo.gl/c2ueJC) where we enable and disable the persistence middleware and the app works without any problem (of course when is disabled we don't have disk persistence). This is because at the end, middleware is subscribed to handle actions, if is not subscribed then it won't handle (and create) actions, but that's all, our app should be able to work without it (of course, in many cases, middleware is essential).
 <br /> Btw in this sample app we use [Realm](https://realm.io/) as DB, mainly because I wanted to experiment with it and because is really easy to build a small DB with a simple scheme and operations, but we could replace this DB for another really easy (the good thing of having everything decoupled).
...
- **Middleware** (persistence) it runs in its own *middleware (persistence) thread*.

Of course when testing, dealing with threads is a nightmare, so we have mechanisms in the app to execute everything in the main thread in that case.

# Dependencies

We want to have a good architecture that its components are decoupled from each others, se we can have our business logic completely independent from frameworks, we can test each component on an isolated way, we can unit test without the need of work on instrumentation tests that are slow, etc.
<br /> This is key in Clean architecture, and this approach (and Redux and Flux) follow that too. Of course you can mess everything, but if you want (and you should), you can have your components decoupled so they can be easier to maintain, to test, to extend, etc.
...
- **Middleware** we can have middleware that doesn't have framework dependencies, from instance a network layer, or with dependencies, for instance a DB where usually in Android we need the *Context*.

# Testing

Now that we know the dependencies of our different components we can say how we can test them:
- **View** here we should have instrumentation/ui tests that test that our interactions have gone well, and what the user sees and expects is correct. I haven't implemented UI tests in this sample since I wanted to focus more in the rest of the tests and components.
...
- **Middleware** it depends if it has framework dependencies or not, but in our sample app that we have a persistence middleware that needs the Android context, we can use instrumentation test for the classes that use framework dependencies or unit test for the classes that don't have dependencies.
